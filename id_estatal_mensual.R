...
##VÍCTIMAS
victimas_estado <- victimas %>% 
  filter(clave_entidad == clave_ent) %>% 
  mutate(mes = enero) %>% 
  select(c(1:9), -c(10:21), mes)

#Víctimas de homicidio doloso por mes
victimas_homicidio_mes <- victimas_estado %>%          
         filter(subtipo == 'Homicidio doloso') %>% 
  group_by(anio) %>% 
  summarise(delitos = sum(mes))

g_victimas_homicidio_mes <- victimas_homicidio_mes %>% 
  ggplot(aes(x = anio, y = delitos))+
  geom_col(colour = '#162342', fill = '#162342')+
  labs(title="Homicidio doloso (víctimas)",
       subtitle = paste(estado, anio_mes, sep = ', '),
       caption = "Fuente: elaboración propia con información del Sistema Nacional de Seguridad Pública",
       x="Año", y="Víctimas de homicidio doloso", size = 4)+
  geom_text(data = victimas_homicidio_mes,aes(label=delitos, vjust = -0.5, hjust = 0.5))

png(filename =  paste0(estado,'_homicidio_victimas_mes','.png'),
    width = 4096, height = 2048, units = "px", pointsize = 12,
    bg = "white",  res = 300)
print(g_victimas_homicidio_mes)
dev.off()


#Víctimas de feminicidio por mes
victimas_feminicidio_mes <- victimas_estado %>%          
  filter(tipo == 'Feminicidio') %>% 
  group_by(anio) %>% 
  summarise(delitos = sum(mes))

g_victimas_feminicidio_mes <- victimas_feminicidio_mes %>% 
  ggplot(aes(x = anio, y = delitos))+
  geom_col(colour = '#162342', fill = '#162342')+
  labs(title="Feminicidio (víctimas)",
       subtitle = paste(estado, anio_mes, sep = ', '),
       caption = "Fuente: elaboración propia con información del Sistema Nacional de Seguridad Pública",
       x="Año", y="Víctimas de feminicidio", size = 4)+
  geom_text(data = victimas_feminicidio_mes,aes(label=delitos, vjust = -0.5, hjust = 0.5))

png(filename =  paste0(estado,'_feminicidio_victimas_mes','.png'),
    width = 4096, height = 2048, units = "px", pointsize = 12,
    bg = "white",  res = 300)
print(g_victimas_feminicidio_mes)
dev.off()

##FEMINICIDIO
#Crea el data set del mes en curso y colpasa por estados.
feminicidio <- victimas %>%
  filter(subtipo == 'Feminicidio',
         anio == anio_filter) %>% 
  group_by(clave_entidad) %>% 
  mutate(mes = enero) %>% 
...
g_arbol_feminicidio <- feminicidio %>%
  ...
  bbc_style()+
  geom_treemap_text(colour = "white", place = "topleft", size = 12, grow = FALSE)+
  labs(title = "Distribución de las víctimas de feminicidio por estado como porcentaje del total nacional",
       subtitle = paste(estado, anio_mes, sep = ', '),
       caption =  "Fuente: elaboración propia con información del Sistema Nacional de Seguridad Pública") +
  scale_fill_brewer(palette = "Blues")+
  theme_minimal()

png(filename =  paste0(estado,'_feminicidio_victimas_arbol','.png'),
    width = 4096, height = 2048, units = "px", pointsize = 12,
    bg = "white",  res = 300)
print(g_arbol_feminicidio)
dev.off()
...
g_tasa_feminicidio <- feminicidio %>%
  ...
  scale_fill_manual(values = c('0' = '#00a5db', '1' = '#162342'), guide = FALSE)+
  coord_flip()+
  labs(title="Víctimas de feminicidio por cada 10 mil habitantes",
       subtitle = paste(estado, anio_mes, sep = ', '),
       caption =  "Fuente: elaboración propia con información del Sistema Nacional de Seguridad Pública",
       x="Estado", y="Tasa de incidencia", size = 4)+
  geom_text(data=feminicidio,aes(label=tasa, vjust = 0.5, hjust = 1), color = 'white')+
...
png(filename =  paste0(estado,'_feminicidio_victimas_tasa','.png'),
    width = 4096, height = 2048, units = "px", pointsize = 12,
    bg = "white",  res = 300)
print(g_tasa_feminicidio)
dev.off()
...
g_total <- feminicidio %>%
  ...
  scale_fill_manual(values = c('0' = '#00a5db', '1' = '#162342'), guide = FALSE)+
  coord_flip()+
  labs(title="Víctimas de feminicidio por entidad",
       subtitle = paste(estado, anio_mes, sep = ', '),
       caption =  "Fuente: elaboración propia con información del Sistema Nacional de Seguridad Pública",
       x="Estado", y="Carpetas de investigación", size = 4)+
  geom_text(data=feminicidio,aes(label=delitos, vjust = 0.5, hjust = 1), color = 'white')+
...
png(filename =  paste0(estado,'_feminicidio_victimas_total','.png'),
    width = 4096, height = 2048, units = "px", pointsize = 12,
    bg = "white",  res = 300)
print(g_total)
dev.off()
...
g_arbol_homicidio <- homicidio %>%
  ...
  bbc_style()+
  geom_treemap_text(colour = "white", place = "topleft", size = 12, grow = FALSE)+
  labs(title = "Distribución de las víctimas de homicidio por estado como porcentaje del total nacional",
       subtitle = paste(estado, anio_mes, sep = ', '),
       caption =  "Fuente: elaboración propia con información del Sistema Nacional de Seguridad Pública") +
  scale_fill_brewer(palette = "Blues")+
  theme_minimal()
...
g_tasa_homicidio <- homicidio %>%
  ...
  scale_fill_manual(values = c('0' = '#00a5db', '1' = '#162342'), guide = FALSE)+
  coord_flip()+
  labs(title="Víctimas de homicidio por cada 10 mil habitantes",
       subtitle = paste(estado, anio_mes, sep = ', '),
       caption =  "Fuente: elaboración propia con información del Sistema Nacional de Seguridad Pública",
       x="Estado", y="Tasa de incidencia", size = 4)+
  geom_text(data=homicidio,aes(label=tasa, vjust = 0.5, hjust = 1), color = 'white')+
...
png(filename =  paste0(estado,'_homicidio_victimas_tasa','.png'),
    width = 4096, height = 2048, units = "px", pointsize = 12,
    bg = "white",  res = 300)
print(g_tasa_homicidio)
dev.off()
...
g_total <- homicidio %>%
  ...
  scale_fill_manual(values = c('0' = '#00a5db', '1' = '#162342'), guide = FALSE)+
  coord_flip()+
  labs(title="Víctimas de homicidio por entidad",
       subtitle = paste(estado, anio_mes, sep = ', '),
       caption =  "Fuente: elaboración propia con información del Sistema Nacional de Seguridad Pública",
       x="Estado", y="Carpetas de investigación", size = 4)+
  geom_text(data=homicidio,aes(label=delitos, vjust = 0.5, hjust = 1), color = 'white')+
...
png(filename =  paste0(estado,'_homicidio_victimas_total','.png'),
    width = 4096, height = 2048, units = "px", pointsize = 12,
    bg = "white",  res = 300)
print(g_total)
dev.off()
